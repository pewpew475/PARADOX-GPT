        ...
        if (metadata && metadata.generated_by && role === 'assistant') {
            ...
            footer.innerHTML = `
                ...
                <div class="message-actions">
                    <button class="message-action-btn copy-message-btn" onclick="copyMessage(this)" title="Copy this message">
            ...
            render() {
                if (this.state.hasError) {
                    return React.createElement('div', { className: 'error' },
                        ...
                        React.createElement('br'),
                        React.createElement('small', {}, 'Check the console for more details.')
                    );
                }
...
`;
        ...
        // Get all React/JavaScript code
        let componentCode = '';
        if (codeBlocks.javascript && codeBlocks.javascript.length > 0) {
            componentCode = codeBlocks.javascript.join('\n\n');
        } else if (triggerContent) {
            componentCode = triggerContent;
        }
            ...
            if (!ComponentToRender) {
                ComponentToRender = function DefaultComponent() {
                    return React.createElement('div', {
                        ...
                        React.createElement('p', {}, 'No React component found.'),
                        ...
                        }, 'Make sure your component is exported or defined as a function starting with a capital letter.')
                    );
                };
            }
...
<body>
    ...
    // Global function for copying individual messages
    ...
    // Global function for copying all assistant answers
    window.copyAllAnswers = function() {
        const assistantMessages = document.querySelectorAll('.message.assistant .message-content');
        if (assistantMessages.length === 0) {
            showToast('No assistant messages found', 'warning');
            return;
        }
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(allAnswers).then(() => {
                showToast(`Copied ${assistantMessages.length} answers to clipboard!`, 'success');
            }).catch(() => {
                showToast('Failed to copy all answers', 'error');
            });
        } else {
            // Fallback method
            const textArea = document.createElement('textarea');
            textArea.value = allAnswers;
            textArea.style.position = 'fixed';
            textArea.style.left = '-999999px';
            document.body.appendChild(textArea);
            textArea.focus();
    ...
    // Simple toast notification function for web
    ...
    return (
        <div style={{ padding: '30px', maxWidth: '600px', margin: '0 auto', fontFamily: 'Arial' }}>
            <h1 style={{ color: '#61dafb', marginBottom: '20px', textAlign: 'center' }}>{message}</h1>

            {/* Counter Section */}
            <div style={{ marginBottom: '30px', textAlign: 'center', padding: '20px', background: '#f8f9fa', borderRadius: '8px' }}>
                <h3 style={{ marginTop: 0 }}>Counter Demo</h3>
                <p style={{ fontSize: '18px' }}>Count: <strong style={{ color: '#61dafb' }}>{count}</strong></p>
                <button
            ...
            <div style={{ marginBottom: '30px', padding: '20px', background: '#f8f9fa', borderRadius: '8px' }}>
                <h3 style={{ marginTop: 0 }}>Message Editor</h3>
                <input
                    type="text"
                    value={message}
        ...
        console.log('Testing React preview with interactive demo...');
    ...
    window.testReactDetection = function() {
        const testCodes = [
            'function App() { return <div>Hello</div>; }',
            'const MyComponent = () => { const [state, setState] = useState(0); return <div>{state}</div>; }',
            'export default function Component() { return React.createElement("div", {}, "Hello"); }',
            'function regularFunction() { console.log("not react"); }',
            'const data = { name: "test" };'
        ];

        console.log('Testing React detection...');
        testCodes.forEach((code, index) => {
            const mockElement = { textContent: code };
            const detected = detectCodeLanguage(mockElement, code);
            console.log(`Test ${index + 1}: "${code.substring(0, 30)}..." -> ${detected}`);
        });
    };
});